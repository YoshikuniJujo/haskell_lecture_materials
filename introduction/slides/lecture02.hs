...
main = runLecture [
	...
	symbols,
	equal, enter, plus, equalPlus, minus, asterisk, slash, circumflex, tilde,
...
prelude = [\t -> do
	...
	text t "* そのためには", \t -> do
	itext t 1 "まずはレクチャー用の環境に慣れてもらうと効率的", \t -> do
...
keyboard = [\t -> do
	...
	text t "* vimエディタのなかでHHKBの操作を学ぶ", \t -> do
...
editor = [\t -> do
	...
	text t "* nanoやemacsを選ぶ人に対しては", \t -> do
	itext t 1 "「いつものように編集してください」と言っておこう", \t -> do
	text t "* 僕はvim userなのでvimについてなら教えられる", \t -> do
	text t "* どれも使ったことがないという人はvimを選ぼう"
 ]
...
enter = [\t -> do
	writeTopTitle t "改行"
	text t "", \t -> do
	text t "* エンターキーで改行しておこう"
	itext t 1 "- エンターキーは右手小指を2こ右へずらすとある", \t -> do
	itext t 1 "- 今後は「次」の練習に行く前に必ずエンターを押す" ]
...
slash = [\t -> do
	...
	itext t 1 "// C-style single-line comment", \t -> do
...
save = [\t -> do
	...
	text t "* まずは挿入モードを抜けるためにESCキーを入力する", \t -> do
	itext t 1 "- ESCキーは左手小指を左上に動かすとある", \t -> do
	text t "* 保存は':' -> 'w' -> 'q' -> ENTER"
 ]
